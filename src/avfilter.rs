#[link(name = "avfilter", vers = "3.42.103")];
#[license = "MIT"];
#[crate_type = "lib"];
/* automatically generated by rust-bindgen */
extern mod avutil;
use std::libc::{c_void,c_int,c_uint,c_schar,uint8_t,int64_t,uint64_t,int8_t,c_uchar};
mod dummy {
    #[link_args = "-lavfilter"]
    extern {}
}
...
externfn!(fn avfilter_version() -> c_uint)
externfn!(fn avfilter_configuration() -> *c_schar)
externfn!(fn avfilter_license() -> *c_schar)
externfn!(fn avfilter_get_class() -> *avutil::AVClass)
externfn!(fn avfilter_copy_buffer_ref_props(dst: *mut AVFilterBufferRef,
                                          src: *mut AVFilterBufferRef))
externfn!(fn avfilter_ref_buffer(_ref: *mut AVFilterBufferRef, pmask: c_int) ->
     *mut AVFilterBufferRef)
externfn!(fn avfilter_unref_buffer(_ref: *mut AVFilterBufferRef))
externfn!(fn avfilter_unref_bufferp(_ref: *mut *mut AVFilterBufferRef))
externfn!(fn avfilter_ref_get_channels(_ref: *mut AVFilterBufferRef) -> c_int)
externfn!(fn avfilter_pad_get_name(pads: *mut AVFilterPad, pad_idx: c_int) ->
     *c_schar)
externfn!(fn avfilter_pad_get_type(pads: *mut AVFilterPad, pad_idx: c_int) ->
     avutil::Enum_AVMediaType)
externfn!(fn avfilter_link(src: *mut AVFilterContext, srcpad: c_uint,
                         dst: *mut AVFilterContext, dstpad: c_uint) -> c_int)
externfn!(fn avfilter_link_free(link: *mut *mut AVFilterLink))
externfn!(fn avfilter_link_get_channels(link: *mut AVFilterLink) -> c_int)
externfn!(fn avfilter_link_set_closed(link: *mut AVFilterLink, closed: c_int))
externfn!(fn avfilter_config_links(filter: *mut AVFilterContext) -> c_int)
externfn!(fn avfilter_get_video_buffer_ref_from_arrays(data:
                                                         [*mut uint8_t, ..4u],
                                                     linesize: [c_int, ..4u],
                                                     perms: c_int, w: c_int,
                                                     h: c_int,
     ...
     -> *mut AVFilterBufferRef)
externfn!(fn avfilter_get_audio_buffer_ref_from_arrays(data: *mut *mut uint8_t,
                                                     linesize: c_int,
                                                     perms: c_int,
                                                     nb_samples: c_int,
                                                     sample_fmt:
                                                     ...
                                                     channel_layout: uint64_t)
     -> *mut AVFilterBufferRef)
externfn!(fn avfilter_get_audio_buffer_ref_from_arrays_channels(data:
                                                                  *mut *mut uint8_t,
                                                              linesize: c_int,
                                                              perms: c_int,
                                                              nb_samples:
                                                              ...
                                                              channel_layout:
                                                                  uint64_t) ->
     *mut AVFilterBufferRef)
externfn!(fn avfilter_process_command(filter: *mut AVFilterContext,
                                    cmd: *c_schar, arg: *c_schar,
                                    res: *mut c_schar, res_len: c_int,
                                    flags: c_int) -> c_int)
externfn!(fn avfilter_register_all())
externfn!(fn avfilter_uninit())
externfn!(fn avfilter_register(filter: *mut AVFilter) -> c_int)
externfn!(fn avfilter_get_by_name(name: *c_schar) -> *mut AVFilter)
externfn!(fn av_filter_next(filter: *mut *mut AVFilter) -> *mut *mut AVFilter)
externfn!(fn avfilter_open(filter_ctx: *mut *mut AVFilterContext,
                         filter: *mut AVFilter, inst_name: *c_schar) -> c_int)
externfn!(fn avfilter_init_filter(filter: *mut AVFilterContext, args: *c_schar,
                                opaque: *mut c_void) -> c_int)
externfn!(fn avfilter_free(filter: *mut AVFilterContext))
externfn!(fn avfilter_insert_filter(link: *mut AVFilterLink,
