#[link(name = "avutil", vers = "54.18.100")];
#[license = "MIT"];
#[crate_type = "lib"];
...
externfn!(fn avutil_version() -> c_uint)
externfn!(fn avutil_configuration() -> *c_schar)
externfn!(fn avutil_license() -> *c_schar)
externfn!(fn av_get_media_type_string(media_type: Enum_AVMediaType) -> *c_schar)
externfn!(fn av_get_picture_type_char(pict_type: Enum_AVPictureType) -> c_schar)
externfn!(fn av_strerror(errnum: c_int, errbuf: *mut c_schar,
                       errbuf_size: size_t) -> c_int)
externfn!(fn av_malloc(size: size_t) -> *mut c_void)
externfn!(fn av_realloc(ptr: *mut c_void, size: size_t) -> *mut c_void)
externfn!(fn av_realloc_f(ptr: *mut c_void, nelem: size_t, elsize: size_t) ->
     *mut c_void)
externfn!(fn av_free(ptr: *mut c_void))
externfn!(fn av_mallocz(size: size_t) -> *mut c_void)
externfn!(fn av_calloc(nmemb: size_t, size: size_t) -> *mut c_void)
externfn!(fn av_strdup(s: *c_schar) -> *mut c_schar)
externfn!(fn av_freep(ptr: *mut c_void))
externfn!(fn av_dynarray_add(tab_ptr: *mut c_void, nb_ptr: *mut c_int,
                           elem: *mut c_void))
externfn!(fn av_max_alloc(max: size_t))
externfn!(fn av_memcpy_backptr(dst: *mut uint8_t, back: c_int, cnt: c_int))
externfn!(fn av_reduce(dst_num: *mut c_int, dst_den: *mut c_int, num: int64_t,
                     den: int64_t, max: int64_t) -> c_int)
externfn!(fn av_mul_q(b: AVRational, c: AVRational) -> AVRational)
externfn!(fn av_div_q(b: AVRational, c: AVRational) -> AVRational)
externfn!(fn av_add_q(b: AVRational, c: AVRational) -> AVRational)
externfn!(fn av_sub_q(b: AVRational, c: AVRational) -> AVRational)
externfn!(fn av_d2q(d: c_double, max: c_int) -> AVRational)
externfn!(fn av_nearer_q(q: AVRational, q1: AVRational, q2: AVRational) ->
     c_int)
externfn!(fn av_find_nearest_q_idx(q: AVRational, q_list: *AVRational) -> c_int)
externfn!(fn av_gcd(a: int64_t, b: int64_t) -> int64_t)
externfn!(fn av_rescale(a: int64_t, b: int64_t, c: int64_t) -> int64_t)
externfn!(fn av_rescale_rnd(a: int64_t, b: int64_t, c: int64_t,
                          arg1: Enum_AVRounding) -> int64_t)
externfn!(fn av_rescale_q(a: int64_t, bq: AVRational, cq: AVRational) ->
     int64_t)
externfn!(fn av_rescale_q_rnd(a: int64_t, bq: AVRational, cq: AVRational,
                            arg1: Enum_AVRounding) -> int64_t)
externfn!(fn av_compare_ts(ts_a: int64_t, tb_a: AVRational, ts_b: int64_t,
                         tb_b: AVRational) -> c_int)
externfn!(fn av_compare_mod(a: uint64_t, b: uint64_t, _mod: uint64_t) ->
     int64_t)
externfn!(fn av_rescale_delta(in_tb: AVRational, in_ts: int64_t,
                            fs_tb: AVRational, duration: c_int,
                            last: *mut int64_t, out_tb: AVRational) ->
     int64_t)
externfn!(fn av_log(avcl: *mut c_void, level: c_int, fmt: *c_schar))
externfn!(fn av_vlog(avcl: *mut c_void, level: c_int, fmt: *c_schar,
                   arg1: va_list))
externfn!(fn av_log_get_level() -> c_int)
externfn!(fn av_log_set_level(arg1: c_int))
externfn!(fn av_log_set_callback(arg1:
                                   extern "C" fn
                                       (arg1: *mut c_void, arg2: c_int,
                                        arg3: *c_schar,
                                        arg4: *mut __va_list_tag)))
externfn!(fn av_log_default_callback(ptr: *mut c_void, level: c_int,
                                   fmt: *c_schar, vl: va_list))
externfn!(fn av_default_item_name(ctx: *mut c_void) -> *c_schar)
externfn!(fn av_default_get_category(ptr: *mut c_void) -> AVClassCategory)
externfn!(fn av_log_format_line(ptr: *mut c_void, level: c_int, fmt: *c_schar,
                              vl: va_list, line: *mut c_schar,
                              line_size: c_int, print_prefix: *mut c_int))
externfn!(fn av_log_set_flags(arg: c_int))
externfn!(fn av_get_sample_fmt_name(sample_fmt: Enum_AVSampleFormat) ->
     *c_schar)
externfn!(fn av_get_sample_fmt(name: *c_schar) -> Enum_AVSampleFormat)
externfn!(fn av_get_alt_sample_fmt(sample_fmt: Enum_AVSampleFormat,
                                 planar: c_int) -> Enum_AVSampleFormat)
externfn!(fn av_get_packed_sample_fmt(sample_fmt: Enum_AVSampleFormat) ->
     Enum_AVSampleFormat)
externfn!(fn av_get_planar_sample_fmt(sample_fmt: Enum_AVSampleFormat) ->
     Enum_AVSampleFormat)
externfn!(fn av_get_sample_fmt_string(buf: *mut c_schar, buf_size: c_int,
                                    sample_fmt: Enum_AVSampleFormat) ->
     *mut c_schar)
externfn!(fn av_get_bits_per_sample_fmt(sample_fmt: Enum_AVSampleFormat) ->
     c_int)
externfn!(fn av_get_bytes_per_sample(sample_fmt: Enum_AVSampleFormat) -> c_int)
externfn!(fn av_sample_fmt_is_planar(sample_fmt: Enum_AVSampleFormat) -> c_int)
externfn!(fn av_samples_get_buffer_size(linesize: *mut c_int,
                                      nb_channels: c_int, nb_samples: c_int,
                                      sample_fmt: Enum_AVSampleFormat,
                                      align: c_int) -> c_int)
externfn!(fn av_samples_fill_arrays(audio_data: *mut *mut uint8_t,
                                  linesize: *mut c_int, buf: *uint8_t,
                                  nb_channels: c_int, nb_samples: c_int,
                                  sample_fmt: Enum_AVSampleFormat,
                                  align: c_int) -> c_int)
externfn!(fn av_samples_alloc(audio_data: *mut *mut uint8_t,
                            linesize: *mut c_int, nb_channels: c_int,
                            nb_samples: c_int,
                            sample_fmt: Enum_AVSampleFormat, align: c_int) ->
     c_int)
externfn!(fn av_samples_copy(dst: *mut *mut uint8_t, src: **mut uint8_t,
                           dst_offset: c_int, src_offset: c_int,
                           nb_samples: c_int, nb_channels: c_int,
                           sample_fmt: Enum_AVSampleFormat) -> c_int)
externfn!(fn av_samples_set_silence(audio_data: *mut *mut uint8_t,
                                  offset: c_int, nb_samples: c_int,
                                  nb_channels: c_int,
                                  sample_fmt: Enum_AVSampleFormat) -> c_int)
externfn!(fn av_dict_get(m: *mut AVDictionary, key: *c_schar,
                       prev: *AVDictionaryEntry, flags: c_int) ->
     *mut AVDictionaryEntry)
externfn!(fn av_dict_count(m: *AVDictionary) -> c_int)
externfn!(fn av_dict_set(pm: *mut *mut AVDictionary, key: *c_schar,
                       value: *c_schar, flags: c_int) -> c_int)
externfn!(fn av_dict_parse_string(pm: *mut *mut AVDictionary, str: *c_schar,
                                key_val_sep: *c_schar, pairs_sep: *c_schar,
                                flags: c_int) -> c_int)
externfn!(fn av_dict_copy(dst: *mut *mut AVDictionary, src: *mut AVDictionary,
                        flags: c_int))
externfn!(fn av_dict_free(m: *mut *mut AVDictionary))
